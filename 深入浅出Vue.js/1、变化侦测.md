...
vue3.0使用`Proxy`侦听数据的变化，可以侦听数据的深层元素，可以侦听数组，而`Object.defineProperty`只能监听第一级的子元素。（`Object.defineProperty`同样可以通过监听数组下标来监听数组，而不这么做的原因，犹大的回答是减少性能消耗。）由于Proxy并不支持ie（ie11及以下）。vue2.0还是采用`Object.defineProperty`侦听变化。
